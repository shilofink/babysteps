async function generateSpells(classObject, classLevel) {
    if(!classObject.spellcasting) {
        return "<p>No spellcasting ability</p>"
    }

    // is there a better way than making 300+ calls to the API each time?
    // const spellbook = await generateSpellbook()

    const spellcasting = classLevel.spellcasting
    const choicesPerLevel = Object.values(spellcasting).filter(level => level > 0)
    if(choicesPerLevel.length === 0) {
        return "<p>No spells known</p>"
    }

    const chosenSpells = []
    for(i = 0; i < choicesPerLevel.length; i++){
        for(j = 0; j < choicesPerLevel[i]; j++){
            const randNum = Math.floor(Math.random() * spellbook[i].length)
            const randomSpell = spellbook[i].splice(randNum, 1)
            chosenSpells.push(randomSpell.pop())
        }
    }

    const spellNames = chosenSpells.map(spell => spell.name)

    return `<p>${spellNames.join(', ')}</p>`
}

async function generateSpellbook() {
    const apiSpellList = await apiAsync('spells')
    const fullSpells = await Promise.all( apiSpellList.results.map(async spell => {
        return await apiAsync(`spells/${spell.index}`)}
    ))
    const spellbook = [[], [], [], [], [], [], [], [], [], []]

    fullSpells.map(spell => {
        spellbook[spell.level].push(spell)
    })
